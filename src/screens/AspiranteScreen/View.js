...
const AspiranteView = ({
  ...
  const handleValues = id => {
    setIds(id)
    const res = allAspirante.filter(x => x.id === id)
    if (res) {
      setValues(res[0])
      setOpen(true)
      setPath('/aspirante/create')
    }
  }
  const save = () => {
    saveData()
  ...
  const columns = [
    ...
    { field: 'domicilio', headerName: 'Domicilio', width: 250 },
    {
      field: 'actions',
      type: 'actions',
      headerName: 'Actions',
  ...
  const datos = { rows: allAspirante, columns }
  return (
    <Box
      sx={{
            ...
            <MuiAppBar sx={{ position: 'relative' }}>
              <Toolbar>
                ...
                <Typography
                  ...
                  component="div"
                >
                  {ids ? 'Actualizar Aspirante' : 'Crear Aspirante'}
                </Typography>
